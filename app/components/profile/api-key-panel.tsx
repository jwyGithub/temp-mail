...
export function ApiKeyPanel() {
    ...
    const toggleApiKey = async (id: string, enabled: boolean) => {
        try {
            ...
            setApiKeys(keys => keys.map(key => (key.id === id ? { ...key, enabled } : key)));
        } catch (error) {
            console.error(error);
            toast({
                title: '更新失败',
        ...
        <div className='bg-background rounded-lg border-2 border-primary/20 p-6 space-y-6'>
            <div className='flex items-center justify-between mb-6'>
                ...
                {canManageApiKey && (
                    <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
                        ...
                        <DialogContent>
                            <DialogHeader>
                                <DialogTitle>{newKey ? 'API Key 创建成功' : '创建新的 API Key'}</DialogTitle>
                                {newKey && (
                            ...
                            <DialogFooter>
                                <DialogClose asChild>
                                    <Button variant='outline' onClick={handleDialogClose} disabled={loading}>
                                        {newKey ? '完成' : '取消'}
                                    </Button>
                                </DialogClose>
                                {!newKey && (
            ...
            {!canManageApiKey ? (
                <div className='text-center text-muted-foreground py-8'>
                    ...
                    {config?.adminContact && <p className='mt-2'>管理员联系方式：{config.adminContact}</p>}
                </div>
            ) : (
                        ...
                        <>
                            {apiKeys.map(key => (
                                <div key={key.id} className='flex items-center justify-between p-4 rounded-lg border bg-card'>
                                    ...
                                    <div className='flex items-center gap-2'>
                                        <Switch checked={key.enabled} onCheckedChange={checked => toggleApiKey(key.id, checked)} />
