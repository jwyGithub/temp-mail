...
export async function GET(request: Request) {
    ...
    try {
        ...
        if (cursor) {
            ...
            conditions.push(
                or(lt(emails.createdAt, new Date(timestamp)), and(eq(emails.createdAt, new Date(timestamp)), lt(emails.id, id)))
            );
        }
        ...
        const nextCursor = hasMore ? encodeCursor(results[PAGE_SIZE - 1].createdAt.getTime(), results[PAGE_SIZE - 1].id) : null;
