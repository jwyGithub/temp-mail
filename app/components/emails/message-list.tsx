    ...
    const fetchMessages = async (cursor?: string) => {
        try {
            ...
            if (!cursor) {
                ...
                const lastDuplicateIndex = newMessages.findIndex(newMsg => oldMessages.some(oldMsg => oldMsg.id === newMsg.id));
        ...
        <>
            <div className='h-full flex flex-col'>
                <div className='p-2 flex justify-between items-center border-b border-primary/20'>
                    ...
                    <span className='text-xs text-gray-500'>{total > 0 ? `${total} 封邮件` : '暂无邮件'}</span>
                </div>
                ...
                <div className='flex-1 overflow-auto' onScroll={handleScroll}>
                    {loading ? (
                        ...
                        <div className='divide-y divide-primary/10'>
                            ...
                            {loadingMore && <div className='text-center text-sm text-gray-500 py-2'>加载更多...</div>}
                        </div>
                    ) : (
                        <div className='p-4 text-center text-sm text-gray-500'>暂无邮件</div>
                    )}
                </div>
            </div>
            <AlertDialog open={!!messageToDelete} onOpenChange={() => setMessageToDelete(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>确定要删除邮件 {messageToDelete?.subject} 吗？</AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
