...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
    ...
    try {
        ...
        if (!email) {
            return NextResponse.json({ error: '邮箱不存在或无权限删除' }, { status: 403 });
        }
    ...
    try {
        ...
        if (messageType === 'sent') {
            ...
            if (!permissionResult.canSend) {
                return NextResponse.json({ error: permissionResult.error || '您没有查看发送邮件的权限' }, { status: 403 });
            }
        }
        ...
        const baseConditions = and(
            ...
            messageType === 'sent' ? eq(messages.type, 'sent') : or(ne(messages.type, 'sent'), isNull(messages.type))
        );
        ...
        if (cursorStr) {
            ...
            conditions.push(or(lt(orderByTime, new Date(timestamp)), and(eq(orderByTime, new Date(timestamp)), lt(messages.id, id))));
        }
        ...
        const nextCursor = hasMore
            ? encodeCursor(
                  messageType === 'sent' ? results[PAGE_SIZE - 1].sentAt!.getTime() : results[PAGE_SIZE - 1].receivedAt.getTime(),
