            ...
            <div className='space-y-4'>
                <div className='flex gap-4'>
                    <div className='flex-1'>
                        <Input value={searchText} onChange={e => setSearchText(e.target.value)} placeholder='输入用户名或邮箱' />
                    </div>
                ...
                <Button onClick={handleAction} disabled={loading || !searchText.trim()} className='w-full'>
                    {loading ? <Loader2 className='w-4 h-4 animate-spin' /> : `设为${roleNames[targetRole]}`}
                </Button>
            </div>
        </div>
    );
