...
export async function POST(request: Request) {
    ...
    try {
        if (userRole !== ROLES.EMPEROR) {
            const maxEmails = config?.maxEmails || EMAIL_CONFIG.MAX_ACTIVE_EMAILS.toString();
            ...
            if (Number(activeEmailsCount[0].count) >= Number(maxEmails)) {
                return NextResponse.json({ error: `已达到最大邮箱数量限制 (${maxEmails})` }, { status: 403 });
            }
        }
        ...
        const domainString = config?.emailDomains;
        ...
        const now = new Date();
        const expires = expiryTime === 0 ? new Date('9999-01-01T00:00:00.000Z') : new Date(now.getTime() + expiryTime);
        ...
        const result = await db.insert(emails).values(emailData).returning({ id: emails.id, address: emails.address });
